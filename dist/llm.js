...
// ENTRY POINT
// ---------------------------------------------------------------------------
export async function chat(messages, tools, context, onToolCall) {
...
// ---------------------------------------------------------------------------
async function chatOpenAICompatible(client, model, messages, tools, context, onToolCall) {
...
// OPENAI
// ---------------------------------------------------------------------------
async function chatOpenAI(messages, tools, context, onToolCall) {
    const client = new OpenAI({
        apiKey: config.llm.openai.apiKey,
    });
    return chatOpenAICompatible(client, config.llm.openai.model, messages, tools, context, onToolCall);
}
// ---------------------------------------------------------------------------
// MISTRAL (OpenAI-compatible)
// ---------------------------------------------------------------------------
async function chatMistral(messages, tools, context, onToolCall) {
    const client = new OpenAI({
        apiKey: config.llm.mistral.apiKey,
        baseURL: config.llm.mistral.baseUrl, // e.g. https://api.mistral.ai/v1
    });
    return chatOpenAICompatible(client, config.llm.mistral.model, messages, tools, context, onToolCall);
}
// ---------------------------------------------------------------------------
// ANTHROPIC
// ---------------------------------------------------------------------------
async function chatAnthropic(messages, tools, context, onToolCall) {
    ...
    for (let i = 0; i < MAX_TOOL_ITERATIONS; i++) {
        ...
        for (const block of toolUses) {
            ...
            if (!tool)
                continue;
            const finalArgs = buildToolArgs(block.name, (block.input ?? {}), tool, context);
            validateArgs(tool, finalArgs);
...
// ---------------------------------------------------------------------------
function buildToolArgs(name, args, tool, context) {
    const enriched = { ...args };
...
// CONTEXT UPDATE
// ---------------------------------------------------------------------------
function updateExecutionContext(toolName, args, result, context) {
    if (!result)
        return;
    if (toolName === "search_hotels") {
        context.hotel.token = result?.result?.token;
        context.hotel.correlationId = result?.result?.correlationId;
        context.hotel.hotels = result?.result?.result.map((h) => ({
            id: h.id,
            name: h.name,
    ...
    if (toolName === "get_rooms_and_rates") {
        ...
        const rooms = result?.result?.rooms?.length > 0 ? result?.result?.rooms : [];
        if (rooms.length > 0) {
    ...
    if (toolName === "revalidate") {
        ...
        if (hotelId === context.hotel.selectedHotel?.hotelId) {
            ...
            if (selectedRoom) {
                const roomFromContext = context.hotel.rooms?.find((r) => r.id === selectedRoom.id);
...
function validateArgs(tool, args) {
    const required = Array.isArray(tool.inputSchema.required)
        ? tool.inputSchema.required.filter((field) => typeof field === "string")
        : [];
            ...
            json =
                json?.result?.result?.length > 0
                    ? {
                        ...
                        result: {
                            ...json.result,
                            result: json?.result?.result.slice(0, 5).map((h) => ({
                                id: h.id,
                                name: h.name,
                                providerName: h.providerName,
                                starRating: h.starRating,
...
function buildContextPrompt(context) {
    ...
    const flightSummaries = Array.isArray(flight.flights)
        ? flight.flights
            ...
            const flightName = (item?.flights || []).map((f) => `Departure: ${f.departure} (departureTime: ${f.departureTime}) -> Arrival: ${f.arrival} (arrivalTime: ${f.arrivalTime})`).join(", ") ?? "Unknown flight";
    ...
    const carSummaries = Array.isArray(context.car.cars)
        ? context.car.cars.slice(0, 5).map((item, index) => {
...
function handleToolResult(json, toolName) {
    ...
    if (toolName === "get_hotel_details" && json) {
        ...
        updatedResult = hotelResult
            ? {
                ...
                nearByAttractions: hotelResult?.nearByAttractions?.length > 0
                    ? hotelResult.nearByAttractions
                        .slice(0, 3)
                        .map((a) => `${a.name} (${a.distance} ${a.unit})`)
                    : [],
                ...
                address: hotelResult?.contact?.address
                    ? [
                        ...
                        .join(", ")
                    : "",
                facilities: hotelResult?.facilities?.length > 0
                    ? hotelResult.facilities.slice(0, 5).map((f) => f.name)
                    : [],
            }
            : json;
    }
